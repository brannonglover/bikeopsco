"use client";

import { useEffect, useState } from "react";
import { useQueryClient } from "@tanstack/react-query";
import { ListSkeleton } from "@/components/ui/Skeleton";
import { BOARD_JOBS_QUERY_KEY } from "@/lib/board-jobs";
import { broadcastJobsRefresh } from "@/lib/jobs-refresh";

type HeldBike = {
  id: string;
  make: string;
  model: string | null;
  bikeType: "REGULAR" | "E_BIKE" | null;
};

type SpamSignal = {
  code: string;
  label: string;
  weight: number;
};

type HeldBooking = {
  id: string;
  firstName: string;
  lastName: string;
  email: string;
  phone: string;
  address: string | null;
  deliveryType: "DROP_OFF_AT_SHOP" | "COLLECTION_SERVICE";
  customerNotes: string | null;
  createdAt: string;
  spamScore: number | null;
  spamSignals: SpamSignal[];
  submittedIp: string | null;
  bikes: HeldBike[];
  serviceNames: string[];
};

function formatWhen(iso: string): string {
  const d = new Date(iso);
  return d.toLocaleString(undefined, {
    month: "short",
    day: "numeric",
    hour: "numeric",
    minute: "2-digit",
  });
}

export default function BookingReviewPage() {
  const queryClient = useQueryClient();
  const [entries, setEntries] = useState<HeldBooking[]>([]);
  const [status, setStatus] = useState<"loading" | "ready" | "error">("loading");
  const [error, setError] = useState<string | null>(null);

  const load = async (opts?: { silent?: boolean }) => {
    if (!opts?.silent) setStatus("loading");
    setError(null);
    try {
      const res = await fetch("/api/held-bookings", { cache: "no-store" });
      if (!res.ok) throw new Error("Failed");
      const data = (await res.json()) as HeldBooking[];
      setEntries(Array.isArray(data) ? data : []);
      setStatus("ready");
    } catch {
      if (!opts?.silent) setStatus("error");
    }
  };

  useEffect(() => {
    void load();
  }, []);

  const removeOptimistically = (id: string) => {
    setEntries((prev) => prev.filter((e) => e.id !== id));
  };

  const act = (
    id: string,
    action: "release" | "discard",
    failureMessage: string
  ) => {
    setError(null);
    const snapshot = entries;
    removeOptimistically(id);

    void (async () => {
      try {
        const res = await fetch(
          `/api/held-bookings/${encodeURIComponent(id)}/${action}`,
          { method: "POST" }
        );
        const data = await res.json();
        if (!res.ok) {
          setEntries(snapshot);
          setError(typeof data?.error === "string" ? data.error : failureMessage);
          return;
        }

        if (action === "release") {
          await queryClient.cancelQueries({ queryKey: BOARD_JOBS_QUERY_KEY });
          await queryClient.invalidateQueries({
            queryKey: BOARD_JOBS_QUERY_KEY,
            refetchType: "all",
          });
          broadcastJobsRefresh({ reason: "held-booking-released" });
        }
      } catch {
        setEntries(snapshot);
        setError(failureMessage);
      }
    })();
  };

  return (
    <div className="w-full min-w-0 space-y-6">
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground mb-1">Held bookings</h1>
          <p className="text-text-secondary">
            {status === "ready"
              ? `${entries.length} booking${entries.length === 1 ? "" : "s"} held as suspected spam`
              : "Booking requests held back before reaching the board."}
          </p>
        </div>
        <button
          type="button"
          onClick={() => void load({ silent: true })}
          className="rounded-lg bg-slate-900 text-white px-3 py-2 text-sm font-semibold hover:bg-slate-800"
        >
          Refresh
        </button>
      </div>

      {error && (
        <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
          {error}
        </div>
      )}

      {status === "loading" && <ListSkeleton rows={3} label="Loading held bookings" />}
      {status === "error" && (
        <div className="rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-600">
          Failed to load held bookings.
        </div>
      )}
      {status === "ready" && entries.length === 0 && (
        <div className="rounded-xl border border-slate-200 bg-white p-6 text-sm text-slate-600">
          Nothing held right now. Suspected spam bookings land here instead of on
          the board — they never create a customer and nobody gets emailed until
          you release them.
        </div>
      )}

      {status === "ready" && entries.length > 0 && (
        <div className="space-y-3">
          {entries.map((e) => {
            const bikesLine =
              e.bikes.length === 1
                ? `${e.bikes[0].make}${e.bikes[0].model ? ` ${e.bikes[0].model}` : ""}`
                : `${e.bikes.length} bikes`;
            const servicesLine = e.serviceNames?.length
              ? e.serviceNames.join(", ")
              : "None specified";

            return (
              <div key={e.id} className="rounded-xl border border-slate-200 bg-white p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                      <h2 className="text-base font-semibold text-slate-900 truncate">
                        {e.firstName} {e.lastName}
                      </h2>
                      <span className="text-xs text-slate-500">
                        {formatWhen(e.createdAt)}
                      </span>
                      <span className="text-xs text-slate-500">
                        ·{" "}
                        {e.deliveryType === "COLLECTION_SERVICE"
                          ? "Collection"
                          : "Drop-off"}
                      </span>
                    </div>

                    <p className="mt-1 text-sm text-slate-700 break-words">
                      <span className="font-semibold">Bikes:</span> {bikesLine}
                    </p>
                    <p className="mt-1 text-sm text-slate-700">
                      <span className="font-semibold">Services:</span> {servicesLine}
                    </p>
                    {e.customerNotes ? (
                      <p className="mt-1 text-sm text-slate-600 whitespace-pre-wrap break-words">
                        <span className="font-semibold text-slate-700">Notes:</span>{" "}
                        {e.customerNotes}
                      </p>
                    ) : null}
                    <p className="mt-2 text-xs text-slate-500 break-words">
                      {e.email} · {e.phone}
                      {e.submittedIp ? ` · from ${e.submittedIp}` : ""}
                    </p>

                    {/* Why it was held, so releasing is a judgement rather than a guess. */}
                    {e.spamSignals.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {e.spamSignals.map((signal) => (
                          <span
                            key={signal.code}
                            className="rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-xs text-amber-800"
                          >
                            {signal.label}
                          </span>
                        ))}
                      </div>
                    )}
                  </div>

                  <div className="flex flex-row sm:flex-col gap-2 shrink-0">
                    <button
                      type="button"
                      onClick={() =>
                        act(e.id, "release", "Failed to release the booking.")
                      }
                      className="rounded-lg px-3 py-2 text-sm font-semibold transition-colors bg-emerald-600 text-white hover:bg-emerald-700"
                    >
                      Not spam
                    </button>
                    <button
                      type="button"
                      onClick={() =>
                        act(e.id, "discard", "Failed to discard the booking.")
                      }
                      className="rounded-lg px-3 py-2 text-sm font-semibold transition-colors bg-slate-100 text-slate-700 hover:bg-slate-200"
                    >
                      Confirm spam
                    </button>
                  </div>
                </div>
              </div>
            );
          })}
        </div>
      )}
    </div>
  );
}

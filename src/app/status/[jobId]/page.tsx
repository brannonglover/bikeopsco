...
export default function StatusPage() {
  ...
  const [smsConsentNotice, setSmsConsentNotice] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/jobs/${jobId}`, { cache: "no-store" })
      ...
      .then((data: StatusJob) => {
        setJob(data);
        setSmsConsent(Boolean(data.customer?.smsConsent));
      })
      .catch(() => setError("Job not found"))
      .finally(() => setLoading(false));
  }, [jobId]);

  async function saveSmsConsentPreference() {
    if (!jobId) return;
    setSavingSmsConsent(true);
    setSmsConsentError(null);
    setSmsConsentNotice(null);

    try {
      const res = await fetch(`/api/jobs/${jobId}/sms-consent`, {
        method: "PATCH",
      ...
      if (!res.ok) {
        setSmsConsentError(
          typeof data?.error === "string"
            ? data.error
            : "Could not update text message preferences."
        );
        return;
      }
        ...
        prev
          ? {
              ...
              customer: prev.customer
                ? { ...prev.customer, smsConsent: Boolean(data?.smsConsent) }
                : prev.customer,
            }
          : prev
      );
      ...
      setSmsConsentNotice(
        data?.smsConsent
          ? "Text updates are on for this repair."
          : "Text updates are off. We’ll keep using email instead."
      );
    } catch {
      setSmsConsentError("Could not update text message preferences.");
    } finally {
    ...
    <div className="w-full max-w-md space-y-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="text-sm font-semibold text-slate-900">Text updates</h2>
            <p className="mt-1 text-sm text-slate-600">
              Email remains your default contact method. Turn this on if you also want service-related texts about this repair.
            </p>
          </div>
          <span
        ...
        <label className="mt-4 flex cursor-pointer gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
          <input
            type="checkbox"
            checked={smsConsent}
            onChange={(e) => {
          ...
          <span className="text-sm leading-snug text-slate-700">
            I agree to receive service-related text messages about this repair,
        ...
        <p className="mt-3 text-xs text-slate-500">
          {smsConsent
            ? "We can text you about this repair."
            : "Text updates are off right now. We’ll keep using email unless you turn texts on."}
        </p>
        ...
        <button
          type="button"
          onClick={saveSmsConsentPreference}
          disabled={savingSmsConsent || smsConsent === Boolean(job.customer?.smsConsent)}
          className="mt-4 w-full rounded-xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {savingSmsConsent
            ? "Saving..."
            : smsConsent
              ? "Turn text updates on"
              : "Use email only"}
        </button>
      </div>
